...
| Asphalt Legends Unite | Gameloft | 2025 | [`entries/asphalt-legends-unite.md`](entries/asphalt-legends-unite.md) |
...
| Minion Rush | Gameloft | 2021 | [`entries/minion-rush.md`](entries/minion-rush.md) |
...
| 2026-09-25 | Named-organization verification pass, per `idm-sweep`'s `docs/RUN_SEARCH_PROMPT.md`, surfaced under Rule 8 while working the vendor-census candidate Gameloft, gated on `TRIAGE_RUBRIC.md` rev `2026-08-24-v1` | 2 — Asphalt Legends Unite, Minion Rush | Asphalt Legends Unite (Hyundai, 2025): all three structural tests settled from Hyundai's own press release (IONIQ 5 N and N Vision 74 added as playable vehicles, four-week in-game tournament) plus The Drum's trade coverage and Gameloft's own newsroom post. Minion Rush (LEGO, 2021): all three structural tests settled from Gameloft's own press release and an independent Universal Games and Digital Platforms release, both describing LEGO brick-themed levels, playable characters, and in-game collectibles added for a time-limited crossover event. `ugc_platform` checked and left unset on both — neither environment is hosted on Fortnite/Minecraft/Roblox/Zepeto. A third candidate on the same environment (Asphalt Legends Unite × Samsung Galaxy Tab S10 Ultra, 2024) no-opped — this index's dedup keys on environment, not environment+brand — but its own sourcing (an independent Livewire case study) surfaced a new vendor-census candidate, Livewire, filed separately. |
